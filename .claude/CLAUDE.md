...
- `oppie/intent.py` — Intent classification. `Intent` enum (`QUESTION`, `INSTRUCTION`, `APPLY`, `AMBIGUOUS`), `classify_intent()` (local heuristics with priority: question > apply > instruction), `classify_intent_llm()` (async, LLM-based with heuristic fallback). Config field `intent_classification` (`local`/`llm`) controls strategy. `_APPLY_MARKERS` regex uses an allowlist pattern for known apply phrases.
...
- `oppie/ask/` — Ask engine package. `engine.py` has `generate_ask()` (async) — uses `run_engine()` with ask-mode steps. Requires LLM backend. Saves ask artifacts and run log entries. `__init__.py` re-exports `generate_ask`.
- `oppie/cli/` — Click CLI package. `__init__.py` uses custom `PromptOrCommand` group class to support bare `oppie "<prompt>"` entry point — unrecognized first positional arg is treated as a prompt. Has `--home`, `--debug`, `--no-sync` flags. `commands/` has one module per command area (`init.py`, `config_cmd.py`, `context.py`, `prompt.py`, `amend.py`, `apply.py`). `apply.py` has the `apply` command with extracted helper functions for each interaction state (error handling, drift resolution, operation display, results). `prompt.py` routes by intent: QUESTION → `_handle_ask()`, INSTRUCTION → `_handle_plan()`, APPLY → `_handle_apply()` (imports helpers from `apply.py`). `console.py` has shared Rich output helpers (`success`, `warn`, `error`, `info`). `extras.py` detects installed optional extras at runtime.
- `oppie/config.py` — YAML config loading/saving, `OppieConfig` and `InstanceType` (local/remote), `IntentClassification` enum. `OppieConfig.llm` is a required `LLMConfig` field (no default). `save_oppie_config()` and `save_provider_credentials()` use atomic writes.
...
- `oppie/plan/` — Plan orchestration package. `engine.py` contains module-level functions for the full plan lifecycle: `generate_plan()` (async, uses `run_engine()` with plan-mode steps, has `save` kwarg defaulting to `True`), `amend_plan()` (async), `check_apply()`, `execute_apply()`, `load_plan()`. Also contains `PreApplyCheck` dataclass, preflight validation, drift detection, and plan apply execution. Requires LLM backend — no fallback paths. `__init__.py` re-exports all public functions and `PreApplyCheck`.
