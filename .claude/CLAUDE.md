...
- `tests/cli/conftest.py` has `setup_cli_instance()` which creates a full instance with `.oppie-marker` and config — required for CLI integration tests that go through `Instance.detect()`. Also has `make_and_save_plan()` helper. `FakeStatus` dataclass + `install_fake_status(monkeypatch, console)` helper patch `console.status` to return recorders that log `.update()` calls — used by renderer spinner tests to assert on label content without Rich animation.
...
- `oppie/logging.py` — `configure_logging(debug, home)` sets up stdlib logging. Default root level is `WARNING`; `--debug` promotes to `DEBUG`. `OPPIE_LOG_LEVEL` env var overrides `--debug` (invalid values fall back to `WARNING` with a stderr warning). File output to `{home}/logs/oppie-{timestamp}.log` when an initialized instance exists, explicit `sys.stderr` `StreamHandler` otherwise. `_NOISY_THIRD_PARTY_LOGGERS = ('httpx', 'httpcore', 'urllib3')` are always capped at WARNING, even with `--debug`.
...
- `oppie/tools/` — LLM-callable tool definitions. `base.py` defines `Tool`, `ToolContext`, `ToolResult`. `tickets.py` has `SEARCH_TICKETS_TOOL` and `GET_TICKET_TOOL`. `operations.py` has `PROPOSE_OPERATION_TOOL` (plan mode only, validates against `ProviderCapabilities`). `Tool.to_llm_schema()` emits a provider-agnostic dict with keys `name`, `description`, `schema` (the JSON-Schema object). Adapters translate `schema` to their wire key: `parameters` for OpenAI (`openai_compatible.py`), `input_schema` for Anthropic (`anthropic.py`).
- `oppie/events.py` — Streaming event types. All events are `@dataclass(slots=True)`. `EngineEvent` is a union type alias. Engine-emitted: `StepStartEvent(step_name, is_final)`, `ThinkingEvent`, `TextDeltaEvent(text, step_name, is_final)`, `ToolCallEvent`, `PlanOperationEvent`, `StatsEvent` (has `step_durations: dict[str, float]` populated per step by `run_engine`). `TextDeltaEvent.is_final=True` only for the mode's final step (ask: answer; plan: summary); consumers must gate user-facing rendering and persisted artifacts on `is_final=True` — non-final text is intermediate research chatter. Caller-emitted: `SyncStartEvent`/`SyncDoneEvent` (CLI/TUI layer). Domain result events: `PlanResultEvent(plan)`, `AskResultEvent(result)`. Internal: `_StepDoneEvent` (usage/turns sentinel, intercepted by `run_engine()`, never yielded to callers).
- `oppie/engine.py` — Multi-turn agent loop. `EngineMode` (`ASK`/`PLAN`), `EngineStep` (defines tools, tool_choice, max_turns, `is_final: bool` marking the final step per mode), `EngineResult`. Mode-specific step sequences: plan = research -> propose (forced `propose_operation`) -> summary (`is_final=True`); ask = research -> answer (`is_final=True`). `run_engine()` is an `AsyncGenerator[EngineEvent, None]` that yields events incrementally; `step.name` and `step.is_final` are threaded into every `StepStartEvent` and `TextDeltaEvent`. Text-only steps (summary/answer) use `llm.stream()` for `TextDeltaEvent` chunks; tool steps use `llm.generate()` and also yield a `TextDeltaEvent` when `response.text` is non-empty (tagged `is_final=False` for non-final steps, so the renderer can suppress it). `collect_engine_result()` consumes the generator and returns `(EngineResult, list[EngineEvent])`; only `is_final=True` text contributes to `EngineResult.text`. `_run_step()` is also a generator, yields `_StepDoneEvent` sentinel for usage tracking.
- `oppie/ask/` — Ask engine package. `engine.py` has `generate_ask()` — an `AsyncGenerator[EngineEvent, None]` that re-yields engine events and yields `AskResultEvent` at the end. Only `TextDeltaEvent`s with `is_final=True` contribute to the persisted `AskResult.answer`. Requires LLM backend. Saves ask artifacts and run log entries. `__init__.py` re-exports `generate_ask`.
- `oppie/cli/` — Click CLI package. `__init__.py` uses custom `PromptOrCommand` group class to support bare `oppie "<prompt>"` entry point — unrecognized first positional arg is rewritten by injecting the hidden `handle_prompt` subcommand name before it, so trailing options (e.g. `--force`) bind to that subcommand. Has `--home`, `--debug`, `--no-sync` flags at the root, plus `--version`/`-V` (`@click.version_option(__version__, '-V', '--version', prog_name='oppie')`) which reads directly from `oppie.__version__` rather than `importlib.metadata` — avoids editable-install drift. `PromptOrCommand.parse_args` leaves leading-dash tokens alone so `-V`/`--version` pass through Click's eager handler before the group callback runs. Bare `oppie` (no subcommand) prints help + a TUI install hint (`pip install oppie[tui]`); the TUI itself is ETH-369. `commands/` has one module per command area (`init.py`, `config_cmd.py`, `context.py`, `prompt.py`, `amend.py`, `apply.py`, `show.py`, `history.py`, `state.py`, `health.py`, `repair.py`, `sync.py`). `render.py` defines `RenderMode` (ASK/PLAN), `EventRenderer(mode, console, *, sync_duration, debug=False)` (class-based, per-event handlers like `on_thinking`/`on_text_delta`/`on_tool_call`/`on_plan_operation`/`on_stats` — overridable for TUI subclassing) with `async def consume(events)` that dispatches each `EngineEvent` and stores final domain results on `renderer.ask_result` / `renderer.plan`. `on_text_delta` gates on `event.is_final`: non-final text is suppressed by default; with `debug=True` it renders dim with a `[<step_name>]` prefix via `_render_debug_research_chunk()` (tracked by `_debug_research_active` and closed in `_end_text_block()`). `on_stats` emits one `<step_name> <duration>s` chip per entry in `StatsEvent.step_durations` (replaces the old single `ask Xs`/`plan Xs` total); falls back to mode total only when `step_durations` is empty. `consume()` wraps the dispatch loop in `try/finally` and awaits `self._spinner.aclose()` to cancel the tick task cleanly. Spinner state is delegated to module-private `_SpinnerController`: owns the live `rich.status.Status`, an asyncio tick task that refreshes the label once per second, phase timer (resets on `StepStartEvent`/`ThinkingEvent`/`ToolCallEvent`), current step name, and current tool name + input summary. Label formats: `Thinking... (step: <name>, <N>s)` with ` (no response — check LLM backend?)` appended at `elapsed >= _IDLE_WARNING_SECONDS` (30 s, thinking mode only); `Calling tool: <name> (<summary>) ... <N>s`. Tool calls also print the persistent `[dim]\[tool_name][/dim]` line before swapping the spinner. `_summarize_tool_input()` caps at `_TOOL_INPUT_SUMMARY_MAX` (40 chars) with ellipsis and strips newlines. `renderer.is_spinner_active` property exposes controller state. `render_sync(renderer, home, config, *, no_sync)` is a `@contextmanager` that wraps `setup_provider` and feeds `SyncStartEvent`/`SyncDoneEvent` to the renderer (calls `setup_provider` with `print_sync_success=False` so the renderer owns the success line). `sync.py` is the user-facing `oppie sync` command (distinct from `oppie.sync`): flushes outbox then pulls from Linear, supports `--full` (passes `checkpoint=''`) and `--no-flush`. Exit codes: 0 ok, 1 missing extra, 2 auth, 3 rate limit, 4 network, 5 other LinearAPIError. Deferred imports of `httpx`/`LinearProvider` after extras check. Escape Rich markup brackets in install hints (e.g., `'oppie\[linear]'`). `apply.py` has the `apply` command with extracted helper functions for each interaction state (error handling, drift resolution, operation display, results). `prompt.py` routes by intent: QUESTION → `_handle_ask()`, INSTRUCTION → `_handle_plan()`, APPLY → `_handle_apply()` (imports `run_apply` from `apply.py`). `_handle_ask`/`_handle_plan` accept `debug: bool` (threaded from `ctx.obj['debug']`) and pass it to `EventRenderer(debug=…)`; they use `render_sync` to stream events live (Thinking spinner, text deltas, tool-call indicators, plan ops as they arrive, stats footer). `handle_prompt` accepts a real `--force` flag (apply only); effective force is `flag_force or 'force' in prompt_lower or '--force' in prompt_lower` so the in-prompt UX still works. `show.py` displays plan or apply details by ID (tries plan first, then scans apply artifacts). `history.py` shows run log entries with `--limit`. `state.py` is a Click group with `show` subcommand for instance state summary (file counts, no deserialization). `console.py` has shared Rich output helpers (`success`, `warn`, `error`, `info`). `extras.py` detects installed optional extras at runtime. `provider_setup.py` exposes `setup_provider(home, config, *, no_sync=False, print_sync_success=True)` as a `@contextmanager` yielding `(provider, AutoSyncResult)`. `print_sync_success=False` suppresses only the success line (cached/error lines still print) so `render_sync` can narrate sync via events without double-printing. Dispatches on `config.provider.provider_type`: local builds `LocalProvider`; linear checks the `linear` extra (exits 1 with install hint if missing), constructs a `LocalProvider` cache, and delegates to `create_external_provider(config, home, cache=cache)`. An internal `ExitStack` closes both provider and cache on exit. Falls back to the cache `LocalProvider` on `ValueError` from the factory (e.g., missing API key). Runs `auto_sync()` and prints status (`✓ Synced`/`⚠ Sync failed`/`● Using cached data`). Used by `prompt.py`, `apply.py`, `amend.py` via `with setup_provider(...)` blocks.
...
- `oppie/plan/` — Plan orchestration package. `engine.py` contains module-level functions for the full plan lifecycle: `generate_plan()` (async generator yielding `EngineEvent`s, has `save` kwarg defaulting to `True`), `_consume_plan_generator()` (consumes generator, returns `Plan`), `amend_plan()` (async, uses `_consume_plan_generator()`), `check_apply()`, `execute_apply()`, `load_plan()`. Only `TextDeltaEvent`s with `is_final=True` (summary step) feed `final_text` which is appended to `plan.risks`; research/propose chatter is filtered out. Also contains `PreApplyCheck` dataclass, preflight validation, drift detection, and plan apply execution. Requires LLM backend — no fallback paths. `__init__.py` re-exports all public functions and `PreApplyCheck`.
- `oppie/llm/` — LLM provider abstraction. `base.py` defines the `LLMProvider` ABC, `TokenUsage`, `LLMResponse`, `StreamResult`, `ToolCallRequest`, `ToolCallResult` dataclasses, `LLMNotConfiguredError`, and `LLMHTTPError` (carries `status_code`/`body` attrs) plus the shared async helper `_raise_for_llm_status(resp)` that both providers call instead of `resp.raise_for_status()` — extracts a human-readable detail from OpenAI-shape (`{"error": {"message": "..."}}`), Ollama-shape (`{"error": "..."}`), or non-JSON bodies via `_extract_error_detail()`, and calls `await resp.aread()` for streaming responses before raising. `LLMProvider.generate()` accepts `tools` (list of tool schemas) and `tool_choice` (None/`'any'`/`{'name': '...'}`) for tool-calling. The engine's neutral message format (`role: 'assistant'` with `tool_calls: [{id, name, input}]`, `role: 'tool_results'` with `results: [{tool_call_id, content, is_error}]`) is translated per-backend: `openai_compatible.py` has `_map_to_openai_format()` (fans tool_results out into one `role: 'tool'` message each; prefixes `is_error=True` content with `'Error: '`; encodes `function.arguments` as a JSON string; coerces empty assistant `content` to `None`); `anthropic.py` has `_map_to_anthropic_format()` (fans tool_results into a single user-role message with `tool_result` blocks; preserves `is_error` natively). Both providers apply their translator in both `generate()` and `stream()`. `_sse.py` is a shared SSE parser. `__init__.py` exports types and `create_llm_provider()` factory.
...
- Default level is `WARNING`; bare `oppie "<prompt>"` emits zero log lines so only renderer output reaches the user. `--debug` promotes to `DEBUG`. Noisy HTTP loggers (`httpx`, `httpcore`, `urllib3`) are always capped at WARNING, even with `--debug`.
- Use `logger.info()` for high-level operations (plan generation, apply execution) — visible only with `--debug`. Use `logger.debug()` for internal details (ticket counts, cache refreshes, GraphQL calls). Use `logger.warning()` for recoverable issues (corrupt session files) — visible by default.
- Log format: `%(asctime)s %(name)s %(levelname)s %(message)s`. Use `%s`-style formatting (not f-strings) in log calls for lazy evaluation. Logs route to `sys.stderr` (explicit) so stdout redirects capture only renderer output.
